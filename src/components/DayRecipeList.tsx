...
export const DayRecipeList = ({
  ...
  const handleGeneratePlan = (date: Date) => {
    ...
    setDayPlans(prev => prev.map(plan => plan.date.getTime() === date.getTime() ? {
      ...plan,
      recipes: newRecipes,
      hasGenerated: true
    } : plan));
  ...
  return <div className="min-h-screen" style={{
    backgroundColor: '#F7F7F7'
      ...
      <div className="space-y-8">
        {dayPlans.map((dayPlan, dayIndex) => <div key={dayIndex} className="space-y-2">
            {/* Day Header */}
            <div className="px-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Checkbox checked={dayPlan.hasGenerated} disabled={!dayPlan.hasGenerated || dayPlan.recipes.length === 0} className={!dayPlan.hasGenerated || dayPlan.recipes.length === 0 ? "pointer-events-none border-muted-foreground/30" : ""} />
                <div className="flex flex-col">
                  <h2 className={`text-lg font-semibold ${isToday(dayPlan.date) ? 'text-primary' : 'text-foreground'}`}>
                    {getDateLabel(dayPlan.date)}
                  </h2>
                  {dayPlan.hasGenerated && dayPlan.recipes.length > 0 && <span className="text-sm text-foreground">
              ...
              {dayPlan.hasGenerated && dayPlan.recipes.length > 0 && <Button size="sm" className="rounded-full h-8 w-8 p-0 flex-shrink-0" style={{
            ...
            {dayPlan.hasGenerated && dayPlan.recipes.length > 0 ? <div className="mx-4">
                <div className="bg-white rounded-lg shadow-sm">
                  <div className="p-4">
                    {dayPlan.recipes.map((recipe, recipeIndex) => <RecipeGridCard key={`${recipe.id}-${recipeIndex}`} recipe={recipe} mealType={recipe.mealTypeLabel} onClick={() => handleRecipeClick(recipe)} onAdd={() => onAddRecipe(recipe)} />)}
                  </div>
                ...
                return <div className="flex items-center gap-3">
                          <div className="flex-1 rounded-lg p-3 bg-white" style={{
                    border: '1px solid #ECEBF1'
                            ...
                            <div className="rounded-lg py-2 px-3 mt-2" style={{
                      backgroundColor: '#ecf9ec'
                          ...
                          color: '#339933'
                                ...
                                <div className="h-3 w-3 rounded-full flex items-center justify-center" style={{
                ...
                <Button onClick={() => handleGeneratePlan(dayPlan.date)} className="w-full py-4 text-base" variant="outline">
                  Generar recetas
                </Button>
              </div>}
          </div>)}
      ...
      <RecipeDrawer recipe={selectedRecipe} isOpen={isDrawerOpen} onClose={handleCloseDrawer} onAdd={onAddRecipe} />
    </div>;
};
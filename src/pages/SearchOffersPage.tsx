...
const SearchOffersPage = () => {
  ...
  return <div className="min-h-screen bg-gray-50 relative">
      {/* Header */}
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate('/milista')} className="rounded-full">
      ...
      <div className="flex flex-col h-[calc(100vh-73px)]">
        ...
        <div ref={scrollRef} onScroll={handleScroll} className={`transition-all duration-300 ease-in-out p-4 overflow-y-auto ${isExpanded ? 'h-full' : 'h-1/2'}`}>
          <h2 className="text-base font-medium mb-3 text-gray-900">Ingredientes de tus recetas</h2>
          <div className="space-y-2">
            {groupedIngredients.map((ingredient, index) => <Card key={index} className="bg-white">
                <CardContent className="p-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100">
                      <img src={`https://images.unsplash.com/photo-1618160702438-9b02ab6515c9?w=100&h=100&fit=crop&q=${encodeURIComponent(ingredient.name)}`} alt={ingredient.name} className="w-full h-full object-cover" />
                    </div>
      ...
      {isExpanded && cheapestSupermarket && <div className="fixed bottom-6 left-4 right-4 animate-fade-in">
          <Card className="shadow-lg border-2 border-green-500 cursor-pointer" onClick={() => handleSupermarketClick(cheapestSupermarket.id)}>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg overflow-hidden">
                    <img src={cheapestSupermarket.logo} alt={cheapestSupermarket.name} className="w-full h-full object-cover" />
                  </div>
                  <div>
